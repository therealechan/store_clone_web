...
export function TextShimmerLoader({ text = 'Thinking', className, size = 'md' }: { text?: string; className?: string; size?: 'sm' | 'md' | 'lg' }) {
    ...
    return <div className={cn('bg-[linear-gradient(to_right,var(--muted-foreground)_40%,var(--foreground)_60%,var(--muted-foreground)_80%)]', 'bg-size-[200%_auto] bg-clip-text font-medium text-transparent', 'animate-[shimmer_4s_infinite_linear]', textSizes[size], className)}>{text}</div>
}
